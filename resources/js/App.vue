    ...
    <section v-else class="app-shell">
        ...
        <main class="main">
            ...
            <div class="content">
                ...
                <CustomersView v-if="view === 'customers'" :customers="customers" :busy="busy" @search="loadCustomers" @save="saveCustomer" @edit="editCustomer" @remove="deleteCustomer" @import="importCustomers" @template="downloadTemplate" />
                <BranchesView v-if="view === 'branches'" :branches="branches" :users="users" :busy="busy" @search="loadBranches" @save="saveBranch" @edit="editBranch" @remove="deleteBranch" />
                <PromoCodesView v-if="view === 'promoCodes'" :promo-codes="promoCodes" :customers="customersList" :branches="branchesList" :busy="busy" @search="loadPromoCodes" @save="savePromoCode" @edit="editPromoCode" @remove="deletePromoCode" @generate="generatePromoCode" />
                ...
                <UsersView v-if="view === 'users'" :users="users" :branches="branchesList" :busy="busy" @save="saveUser" />
            </div>
        </main>
    </section>
</template>
...
const loginForm = reactive({ email: 'admin@promo.test', password: 'password' });
...
const canManage = computed(() => ['admin', 'promo_manager'].includes(user.value?.role));
const canReports = computed(() => ['admin', 'promo_manager', 'reports_manager'].includes(user.value?.role));
const navItems = computed(() => {
    const items = [{ view: 'dashboard', label: 'لوحة التحكم' }];
    if (user.value?.role === 'seller') items.push({ view: 'seller', label: 'تفعيل البرومو' });
    if (canManage.value) {
    ...
    if (user.value?.role === 'admin') items.push({ view: 'users', label: 'المستخدمون' });
    return items;
});
...
async function bootstrapView() {
    ...
    if (user.value?.role === 'admin' || canManage.value || canReports.value) await loadUsers();
...
async function loadUsers() {
    users.value = await api(() => axios.get('/api/users'));
}

async function saveUser(payload) {
    await api(() => axios.post('/api/users', payload), 'تم إنشاء المستخدم.');
    await loadUsers();
}
...
const CustomersView = {
    ...
    emits: ['search', 'save', 'edit', 'remove', 'import', 'template'],
        ...
        <section class="grid">
            ...
            <div class="panel table-wrap">
                <table><thead><tr><th>الفرع</th><th>الكود</th><th>المدينة</th><th>البائعون</th><th>الحالة</th><th>إجراءات</th></tr></thead><tbody>
                    <tr v-for="row in branches.data" :key="row.id"><td>{{ row.name }}</td><td>{{ row.code }}</td><td>{{ row.city }}</td><td>{{ (row.users || []).map(u => u.name).join('، ') }}</td><td><span :class="['badge', row.is_active ? 'on' : 'off']">{{ row.is_active ? 'نشط' : 'متوقف' }}</span></td><td class="actions"><button class="btn" @click="pick(row)">تعديل</button><button class="btn danger" @click="$emit('remove', row)">حذف</button></td></tr>
...
const PromoCodesView = {
    ...
    emits: ['search', 'save', 'remove', 'generate'],
...
const UsersView = {
    ...
    emits: ['save'],
    data: () => ({ form: { name: '', email: '', password: 'password', role: 'seller', is_active: true, branch_ids: [] } }),
    components: { EmptyRow },
    template: `
        <section class="grid">
            <div class="panel">
                ...
                <form class="form" @submit.prevent="$emit('save', form)">
                    <label class="field"><span>الاسم</span><input class="input" v-model="form.name" required></label>
                    <label class="field"><span>البريد</span><input class="input" type="email" v-model="form.email" required></label>
                    <label class="field"><span>كلمة المرور</span><input class="input" v-model="form.password" required></label>
                    <label class="field"><span>الدور</span><select class="select" v-model="form.role"><option value="admin">Admin</option><option value="promo_manager">Promo Manager</option><option value="seller">Seller</option><option value="reports_manager">Reports Manager</option></select></label>
                    <div class="field wide"><span>الفروع</span><div class="multi-list"><label v-for="b in branches" class="check-row"><input type="checkbox" :value="b.id" v-model="form.branch_ids"> {{ b.name }}</label></div></div>
                    <label class="check-row"><input type="checkbox" v-model="form.is_active"> نشط</label>
                    <div class="actions wide"><button class="btn primary" :disabled="busy">حفظ المستخدم</button></div>
                </form>
            </div>
            <div class="panel table-wrap">
                <table><thead><tr><th>الاسم</th><th>البريد</th><th>الدور</th><th>الفروع</th><th>الحالة</th></tr></thead><tbody>
                    <tr v-for="u in users" :key="u.id"><td>{{ u.name }}</td><td>{{ u.email }}</td><td>{{ u.role }}</td><td>{{ (u.branches || []).map(b => b.name).join('، ') }}</td><td><span :class="['badge', u.is_active ? 'on' : 'off']">{{ u.is_active ? 'نشط' : 'متوقف' }}</span></td></tr>
                    <EmptyRow v-if="!users.length" :cols="5" />
                </tbody></table>
            </div>
        </section>
    `,
